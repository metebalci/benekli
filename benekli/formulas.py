...
CONSTANT_6d29_CUBE = (6*6*6)/(29*29*29)
...
def XYZ_to_Lab(XYZ, illuminant_XYZ):
    ...
    if (X/Xn) > CONSTANT_6d29_CUBE:
        fx = fc

    else:
        fx = fd

    if (Y/Yn) > CONSTANT_6d29_CUBE:
        fy = fc

    else:
        fy = fd

    if (Z/Zn) > CONSTANT_6d29_CUBE:
        fz = fc
...
def de94_ex(Lab1, Lab2, kL, K1, K2):
    l1 = Lab1[0]
    a1 = Lab1[1]
    b1 = Lab1[2]
    l2 = Lab2[0]
...
def de94_ex_for_graphic_arts(Lab1, Lab2):
    return de_cie_94_ex(Lab1, Lab2, 1.0, 0.045, 0.015)


def de94_ex_for_textiles(Lab1, Lab2):
    return de_cie_94_ex(Lab1, Lab2, 2.0, 0.048, 0.014)


def de00_ex(Lab1, Lab2):
    l1 = Lab1[0]
    a1 = Lab1[1]
    b1 = Lab1[2]
    l2 = Lab2[0]
...
def de76(ref_im, sim_im):
    assert ref_im.mode == "LAB"
    assert sim_im.mode == "LAB"
    ref_data = ref_im.getdata()
    sim_data = sim_im.getdata()
    diff_im = Image.new("RGB", ref_im.size)
    diff_data = diff_im.getdata()
    for i in (0, ref_im.width * ref_im.height):
        ref_pixel = ref_data[i]
        sim_pixel = sim_data[i]
